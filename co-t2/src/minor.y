...
instr		: elifs else FI								{ $$ = binNode(FI, $1, $2); ifDepth --; }	
			...
			| ASSERT expr ';'					{ $$ = uniNode(ASSERT, $2); $$->info = yylineno; }
...
expr		: lval							
			...
			| expr '&' expr                 { $$ = binNode('&', $1, $3); handleBinOp('&', $$, $1, $3); }
			| expr '|' expr                 { $$ = binNode('|', $1, $3); handleBinOp('|', $$, $1, $3); }
			| expr BOR expr					{ $$ = binNode(BOR, $1, $3); handleBinOp(BOR, $$, $1, $3); }
...
void handleBinOp(int type, Node* result, Node* l, Node* r) {
	switch (type) {
		case '^': case '*': case '/': case '%': case '&': case '|': case BOR:
